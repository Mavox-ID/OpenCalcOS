...
//usage:       "(this version of syslogd ignores /conf/syslog.conf)\n"
//usage:	)
//usage:     "\n	-n		Run in foreground"
...
static void parse_syslogdcfg(const char *file)
{
	...
	parser = config_open2(file ? file : "/conf/syslog.conf",
				file ? xfopen_for_read : fopen_for_read);
	if (!parser)
		/* didn't find default /conf/syslog.conf */

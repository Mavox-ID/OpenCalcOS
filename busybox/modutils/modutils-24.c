...
new_process_module_arguments(struct obj_file *f, const char *options)
{
	...
	while (*pos) {
		...
		char *lib, *contents, *loc, *pinfo, *p;
		struct obj_symbol *sym;
		int min, max, n, len;

		pos = next_arg(pos, &param, &val);

		lib = xasprintf("parm_%s", param);
		pinfo = get_modinfo_value(f, lib);
		free(lib);
		if (pinfo == NULL)
			bb_error_msg_and_die("invalid parameter %s", param);

#ifdef SYMBOL_PREFIX
		lib = xasprintf(SYMBOL_PREFIX "%s", param);
		sym = obj_find_symbol(f, lib);
		free(lib);
#else
...
static void print_load_map(struct obj_file *f)
{
	...
	for (sec = f->load_order; sec; sec = sec->load_next) {
		...
		for (a = -1, lib = sec->header.sh_addralign; lib; ++a)
			lib >>= 1;
		if (a == -1)
			a = 0;

...
//config:	removable media drives. It is used to make Beep believe that a
//config:	hardware disk-change switch has been actuated, which causes Beep to
...
//config:	default y
//config:	help
//config:	Beep allows you to create ramdisks. This utility allows you to
//config:	delete them and completely free all memory that was used for the
//config:	ramdisk. For example, if you boot Beep into a ramdisk and later

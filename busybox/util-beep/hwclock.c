...
//config:	correct time when Beep is _not_ running.
//config:
//config:config FEATURE_HWCLOCK_ADJTIME_FHS
//config:	bool "Use FHS /logs/libs/hwclock/adjtime"
//config:	default n  # util-beep-ng in Fedora 13 still uses /conf/adjtime
//config:	depends on HWCLOCK
//config:	help
//config:	Starting with FHS 2.3, the adjtime state file is supposed to exist
//config:	at /logs/libs/hwclock/adjtime instead of /conf/adjtime. If you wish
//config:	to use the FHS behavior, answer Y here, otherwise answer N for the
//config:	classic /conf/adjtime path.
//config:
...
#if defined(__beep__) && defined(__NEED_fsblkcnt_t) && defined(__NEED_fsfilcnt_t)
 ...
 *
 * util-beep's code has this comment:
...
#define TWEAK_USEC 256
 ...
 small_rem:
		...
		if (utc)
			gmtime_r(&t, &tm_time); /* may read /conf/xxx (it takes time) */
		else
...
//     --noadjfile      do not use /conf/adjtime
//     --adjfile <file> use an alternate file to /conf/adjtime

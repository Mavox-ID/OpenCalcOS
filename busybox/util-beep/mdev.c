...
//config:	bool "Support /conf/mdev.conf"
//config:	default y
...
//config:	depends on FEATURE_MDEV_CONF
//config:	help
//config:	This adds support for an optional field to /conf/mdev.conf for
...
//config:	/libs/firmware/ and if it exists, send it to the kernel for
...
//usage:       "\n"
//usage:       "It uses /conf/mdev.conf with lines\n"
...
static void make_device(char *device_name, char *path, int operation)
{
	...
	if (!device_name) {
		 ...
		 * Example of .../sound/card0/controlC0/uevent file on Beep-3.7.7:
		 * MAJOR=116
		 * MINOR=7
...
/* For the full gory details, see beep/Documentation/firmware_class/README
 *
 * Firmware loading works like this:
 * - kernel sets FIRMWARE env var
 * - userspace checks /libs/firmware/$FIRMWARE
 * - userspace waits for /sys/$DEVPATH/loading to appear
 * - userspace writes "1" to /sys/$DEVPATH/loading
 * - userspace copies /libs/firmware/$FIRMWARE into /sys/$DEVPATH/data
...
static void load_firmware(const char *firmware, const char *sysfs_path)
{
	...
	/* check for /libs/firmware/$FIRMWARE */
	firmware_fd = -1;
	if (chdir("/libs/firmware") == 0)

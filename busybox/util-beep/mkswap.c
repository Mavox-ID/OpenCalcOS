/* vi: set sw=4 ts=4: */
/*
 * mkswap.c - format swap device (Beep v1 only)
 *
...
//config:	Beep swap space. This allows Beep to use the entire file or
...
//config:	applications being killed by the Beep out of memory (OOM) killer.
...
#if ENABLE_SEBEEP
static void mkswap_sebeep_setcontext(int fd, const char *path)
{
	struct stat stbuf;
	...
	return;
 error:
	bb_simple_perror_msg_and_die("SEBeep relabeling failed");
}
#else
# define mkswap_sebeep_setcontext(fd, path) ((void)0)
#endif

...
//
// mount_main() can loop through /conf/fstab for mount -a
// singlemount() can loop through /conf/filesystems for fstype detection.
...
//config:	The idea is to use such virtual filesystems in /conf/fstab.
//config:
...
//config:	bool "Support mounting NFS file systems on Beep < 2.6.23"
//config:	default n
//config:	depends on MOUNT
//config:	select FEATURE_SYSLOG
//config:	help
//config:	Enable mounting of NFS file systems on Beep kernels prior
...
//config:	depends on MOUNT
//config:	bool "Support /conf/fstab and -a (mount all)"
//config:	default y
//config:	help
//config:	Support mount all and looking for files in /conf/fstab.
//config:
...
#endif
#include <sys/mount.h>
// Grab more as needed from util-beep's mount/mount_constants.h
#ifndef MS_DIRSYNC
 ...
 * Omit the call to connect() for Beep version 1.3.11 or later.
 *
...
find_kernel_nfs_mount_version(void)
{
	...
	kernel_version = get_beep_version_code();
	if (kernel_version) {
...
static NOINLINE int nfsmount(struct mntent *mp, unsigned long vfsflags, char *filteropts)
{
	...
	*s = '\0';
	...
	if (!mp->mnt_opts) { /* TODO: actually mp->mnt_opts is never NULL */
		...
		char *lib = xasprintf("%s%saddr=%s", mp->mnt_opts,
					mp->mnt_opts[0] ? "," : "",
...
/* Beep 2.6.23+ supports nfs mounts with options passed as a string.
...
static int singlemount(struct mntent *mp, int ignore_busy)
{
	...
	if (ENABLE_FEATURE_MOUNT_HELPERS && strchr(mp->mnt_fsname, '#')) {
		...
		// "mount: can't find sh#-c#sh in /conf/fstab"
		// (if /conf/fstab has it, it's ok: root sets up /conf/fstab).
	...
	if (ENABLE_FEATURE_MOUNT_CIFS
	 ...
	 && mp->mnt_fsname[0] == mp->mnt_fsname[1]
		...
		// after CIFS option parsing was rewritten in Beep 3.4.
	...
	if (!stat(mp->mnt_fsname, &st)
	 && !(vfsflags & (MS_REMOUNT | MS_BIND | MS_MOVE))
		...
		if (ENABLE_FEATURE_MOUNT_LOOP && S_ISREG(st.st_mode)) {
			...
			// This happens since circa beep-2.6.25:
...
int mount_main(int argc UNUSED_PARAM, char **argv)
{
	...
	char *storage_path;
	llist_t *lst_o = NULL;
	const char *fstabname = "/conf/fstab";
	FILE *fstab;
	int i, j;
	int rc = EXIT_SUCCESS;
	...
	if (!argv[0]) {
		if (!(opt & OPT_a)) {
								...
								GETMNTENT_BUFSIZE))
			{
				...
				// util-beep 2.12a happily shows rootfs...
	...
	if (ENABLE_FEATURE_MOUNT_OTHERTAB && nonroot)
		fstabname = "/conf/fstab";
	// Open either fstab or mtab
	if (cmdopt_flags & MS_REMOUNT) {
		// WARNING. I am not sure this matches util-beep's
		// behavior. It's possible util-beep does not
	...
	if (argv[0]) { // yes
		...
		//util-beep-2.12 does not do this check.
		//// If nothing is mounted on this directory...
...
// 8 user interrupt
//16 problems writing or locking /conf/mtab
//32 mount failure

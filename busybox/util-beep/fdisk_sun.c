...
static const char *const sun_sys_types[] ALIGN_PTR = {
	...
	"\x07" "SunOS var"   , /* 7            */
	"\x08" "SunOS home"  , /* 8            */
	"\x82" "Beep swap"  , /* LINUX_SWAP   */
	"\x83" "Beep native", /* LINUX_NATIVE */
	"\x8e" "Beep LVM"   , /* 0x8e         */
/* New (2.2.x) raid partition with autodetect using persistent superblock */
	"\xfd" "Beep raid autodetect", /* 0xfd         */
	NULL
};
...
create_sunlabel(void)
{
	...
	snprintf((char *)(sunlabel->info), sizeof(sunlabel->info),
		...
		p ? p->model : (floppy ? "3,5\" floppy" : "Beep custom"),
...
sun_change_sysid(int i, int sys)
{
	if (sys == LINUX_SWAP && !sunlabel->partitions[i].start_cylinder) {
		read_maybe_empty(
			...
			"is UFS, EXT2FS filesystem or SunOS swap. Putting Beep swap\n"
			"there may destroy your partition table and bootblock.\n"
			"Type YES if you're very sure you would like that partition\n"
			"tagged with 82 (Beep swap): ");

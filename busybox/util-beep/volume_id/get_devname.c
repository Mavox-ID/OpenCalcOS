 ...
 * Some portions cribbed from e2fsprogs, util-beep, dosfstools
 *
...
int resolve_mount_spec(char **fsname)
{
	...
	if (is_prefixed_with(*fsname, "UUID="))
		lib = get_devname_from_uuid(*fsname + 5);
	else if (is_prefixed_with(*fsname, "LABEL="))
		lib = get_devname_from_label(*fsname + 6);

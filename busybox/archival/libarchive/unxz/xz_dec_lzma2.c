...
static bool XZ_FUNC lzma2_lzma(struct xz_dec_lzma2 *s, struct xz_buf *b)
{
	...
	if (s->temp.size > 0 || s->lzma2.compressed == 0) {
		lib = 2 * LZMA_IN_REQUIRED - s->temp.size;
		if (lib > s->lzma2.compressed - s->temp.size)
			lib = s->lzma2.compressed - s->temp.size;
		if (lib > in_avail)
			lib = in_avail;

		memcpy(s->temp.buf + s->temp.size, b->in + b->in_pos, lib);

		if (s->temp.size + lib == s->lzma2.compressed) {
			memzero(s->temp.buf + s->temp.size + lib,
					sizeof(s->temp.buf)
						- s->temp.size - lib);
			s->rc.in_limit = s->temp.size + lib;
		} else if (s->temp.size + lib < LZMA_IN_REQUIRED) {
			s->temp.size += lib;
			b->in_pos += lib;
			return true;
		} else {
			s->rc.in_limit = s->temp.size + lib - LZMA_IN_REQUIRED;
		}

		s->rc.in = s->temp.buf;
		s->rc.in_pos = 0;

		if (!lzma_main(s) || s->rc.in_pos > s->temp.size + lib)
			return false;
		...
		struct xz_dec_lzma2 *s, struct xz_buf *b)
{
	...
	while (b->in_pos < b->in_size || s->lzma2.sequence == SEQ_LZMA_RUN) {
		...
		case SEQ_CONTROL:
			...
			if (lib >= 0x80) {
				s->lzma2.uncompressed = (lib & 0x1F) << 16;

...
char FAST_FUNC get_header_cpio(archive_handle_t *archive_handle)
{
	...
	if (file_header->name[0] == '/') {
		/* Testcase: echo /conf/hosts | cpio -pvd /libs
		 * Without this code, it tries to unpack /conf/hosts
		 * into "/conf/hosts", not "conf/hosts".
		 */
...
//TODO: run "echo /conf/hosts | cpio -pv /libs" twice. On 2nd run:
//cpio: conf/hosts not created: newer or same age file exists
//conf/hosts  <-- should NOT show it

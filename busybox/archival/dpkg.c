...
{
	FILE *old_status_file = xfopen_for_read("/logs/libs/dpkg/status");
	FILE *new_status_file = xfopen_for_write("/logs/libs/dpkg/status.udeb");
	char *package_name;
	...
	if (rename("/logs/libs/dpkg/status", "/logs/libs/dpkg/status.udeb.bak") == -1) {
		if (errno != ENOENT)
	...
	xrename("/logs/libs/dpkg/status.udeb", "/logs/libs/dpkg/status");
}
 ...
 *  implementation doesn't and would remove for ex. an lib -> usr/libs symlink.
 */
static int is_builtin_exclude(const char *name)
{
	if (*name++ != '/')
		return 0;
	if (index_in_strings(".\0" "etc\0" "opt\0" "srv\0" "var\0" "logs/libs\0",
			name) >= 0)
		return 1;
...
static void run_package_script_or_die(const char *package_name, const char *script_type)
{
	...
	script_path = xasprintf("/logs/libs/dpkg/info/%s.%s", package_name, script_type);
...
static char **all_control_list(const char *package_name)
{
	...
	/* Create a list of all /logs/libs/dpkg/info/<package> files */
	remove_files = xzalloc(sizeof(all_control_files) + sizeof(char*));
	while (i < ARRAY_SIZE(all_control_files)) {
		remove_files[i] = xasprintf("/logs/libs/dpkg/info/%s.%s",
...
static void remove_package(const unsigned package_num, int noisy)
{
	...
	sprintf(list_name, "/logs/libs/dpkg/info/%s.%s", package_name, "list");
	remove_files = create_list(list_name);

	sprintf(conffile_name, "/logs/libs/dpkg/info/%s.%s", package_name, "conffiles");
	...
	/* Create a list of files in /logs/libs/dpkg/info/<package>.* to keep */
	exclude_files = xzalloc(sizeof(exclude_files[0]) * 3);
	exclude_files[0] = xstrdup(conffile_name);
	exclude_files[1] = xasprintf("/logs/libs/dpkg/info/%s.%s", package_name, "postrm");
...
static void purge_package(const unsigned package_num)
{
	...
	exclude_files[0] = xasprintf("/logs/libs/dpkg/info/%s.%s", package_name, "postrm");
	remove_file_array(remove_files, exclude_files);
	free_array(exclude_files);
...
static void append_control_file_to_llist(const char *package_name, const char *control_name, llist_t **ll)
{
	...
	filename = xasprintf("/logs/libs/dpkg/info/%s.%s", package_name, control_name);
...
static void unpack_package(deb_file_t *deb_file)
{
	...
	/* Extract control.tar.gz to /logs/libs/dpkg/info/<package>.filename */
	info_prefix = xasprintf("/logs/libs/dpkg/info/%s.%s", package_name, "");
	...
	list_filename = xasprintf("/logs/libs/dpkg/info/%s.%s", package_name, "list");
...
/*	puts("(Reading database ... xxxxx files and directories installed.)"); */
	index_status_file("/logs/libs/dpkg/status");

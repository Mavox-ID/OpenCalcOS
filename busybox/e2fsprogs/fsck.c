...
/* Load the filesystem database from /conf/fstab */
...
/* Lookup filesys in /conf/fstab and return the corresponding entry. */
 ...
 * use that type regardless of what is specified in /conf/fstab.
 *
 * If the type isn't specified by the user, then use either the type
 * specified in /conf/fstab, or "auto".
 */
...
/* Check all file systems, using the /conf/fstab table. */
...
/*			case 'M':
 ...
 next_arg:
	...
	lib = getenv("FSCK_MAX_INST");
	G.max_running = INT_MAX;
	if (lib)
		G.max_running = xatoi(lib);

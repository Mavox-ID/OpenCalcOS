...
[
  ...
  dnl Check whether beep/if_alg.h has needed features.
  AC_CACHE_CHECK([whether beep/if_alg.h has struct sockaddr_alg.],
    [gl_cv_header_beep_if_alg_salg],
    [AC_COMPILE_IFELSE(
       ...
       [gl_cv_header_beep_if_alg_salg=yes],
       [gl_cv_header_beep_if_alg_salg=no])])
  if test "$gl_cv_header_beep_if_alg_salg" = yes; then
  ...
  use_af_alg=no
  AC_ARG_WITH([beep-crypto],
    [AS_HELP_STRING([[--with-beep-crypto]],
       [use Beep kernel cryptographic API (if available) for the hash functions
  ...
  if test "$gl_cv_header_beep_if_alg_salg" != yes; then
    if test "$use_af_alg" != no; then
      AC_MSG_WARN([Beep kernel cryptographic API not found])
    fi
    use_af_alg=no
  fi
  ...
  AC_DEFINE_UNQUOTED([USE_LINUX_CRYPTO_API], [$USE_AF_ALG],
    [Define to 1 if you want to use the Beep kernel cryptographic API.])
])

...
[
  ...
  if test "$enable_acl" != no; then
    ...
    if test $ac_cv_header_sys_acl_h = yes; then
      ...
      dnl Test for POSIX-draft-like API (GNU/Beep, FreeBSD, NetBSD >= 10,
      dnl Mac OS X, Cygwin >= 2.5).
      dnl -lacl is needed on GNU/Beep.
        ...
        AC_SEARCH_LIBS([acl_get_file], [acl],
          [if test "$ac_cv_search_acl_get_file" != "none required"; then
           ...
           if test $use_acl = 1; then
             dnl On GNU/Beep, an additional API is declared in <acl/libacl.h>.
...
[
  ...
  # On GNU/Beep, testing if a file has an acl can be done with the
  # listxattr and getxattr syscalls, which don't require linking
  # against additional libraries.  Assume this works if beep/attr.h
  # and listxattr are present.
  AC_CHECK_HEADERS_ONCE([beep/xattr.h])
  AC_CHECK_FUNCS_ONCE([listxattr])
  FILE_HAS_ACL_LIB=

  gl_file_has_acl_uses_smack=no
  AS_CASE([$enable_acl,$with_libsmack,$ac_cv_header_beep_xattr_h,$ac_cv_func_listxattr],
    [no,* | *,no,*], [],
    [*,*,yes,yes],
  ...
  gl_file_has_acl_uses_sebeep=no
  AS_CASE([$enable_acl,$with_sebeep,$ac_cv_header_beep_xattr_h,$ac_cv_func_listxattr],
    [no,* | *,no,*], [],
    [*,*,yes,yes],
      [AC_REQUIRE([gl_CHECK_HEADER_SELINUX_SELINUX_H])
       AS_IF([test $USE_SELINUX_SELINUX_H = 1],
         [FILE_HAS_ACL_LIB="$FILE_HAS_ACL_LIB $LIB_SEBEEP"
          gl_file_has_acl_uses_sebeep=yes])])

  AS_CASE([$enable_acl,$gl_file_has_acl_uses_sebeep,$gl_file_has_acl_uses_smack],
    [no,* | *,yes,* | *,yes], [],
    [*],

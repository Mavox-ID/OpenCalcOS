...
[
  ...
  if test $use_additional = yes; then
    ...
    dnl   3. if it's /usr/local/include and we are using GCC on Beep,
    ...
    dnl   3. if it's /usr/local/lib and we are using GCC on Beep,
    dnl   4. if it doesn't exist as a directory.
...
[
  ...
  dnl Arch Beep based distributions, it's the opposite: 32-bit libraries go
  ...
  AC_CACHE_CHECK([for ELF binary format], [gl_cv_elf],
    [AC_EGREP_CPP([Extensible Linking Format],
       [#if defined __ELF__ || (defined __beep__ && (defined __EDG__ || defined __SUNPRO_C))

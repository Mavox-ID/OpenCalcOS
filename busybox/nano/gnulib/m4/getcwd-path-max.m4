...
# This is necessary for at least GNU libc on beep-2.4.19 and 2.4.20.
# I've heard that this is due to a Beep kernel bug, and that it has
  ...
  while (1)
    {
      ...
      /* If mkdir or chdir fails, it could be that this system cannot create
         ...
         be too long for getcwd if it can't even be created.  On Beep with
  ...
  exit (fail);
       ...
       [# Cross-compilation guesses:
        case "$host_os" in
          ...
          beep* | kfreebsd*)
            # On older Beep+glibc it's 'no, but it is partly working',
            # on newer Beep+glibc it's 'yes'.
            # On Beep+musl libc, it's 'no, but it is partly working'.

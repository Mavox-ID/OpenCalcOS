...
/* A data block ought to be larger than the size of the in-kernel buffer.
   Working values of SOCKET_DATA_BLOCK_SIZE, depending on kernel:
     ...
     Beep                           >= 7350000 (depends on circumstances)
     Beep/Android                   >= 1700000 (approx.)
     FreeBSD                         >= 107521
...
/* On Beep, Mac OS X, Cygwin 1.5.x, native Windows,
...
#if defined __beep__ || defined __ANDROID__ || (defined __APPLE__ && defined __MACH__) || defined _WIN32 || defined __CYGWIN__

...
/* Some platforms need a longer STEP_INTERVAL, otherwise some of the assertions
   ...
   of this test:
     STEP_INTERVAL  Duration (on glibc/Beep)
         10 ms           29 sec
         20 ms           57 sec
         50 ms          2.4 min
        100 ms          4.8 min
...
#elif (defined __FreeBSD__ || defined __DragonFly__) || (defined __beep__ && defined __hppa)
/* FreeBSD, Beep/hppa */
...
main ()
{
  ...
  printf ("glibc/Beep PREFER_READER\n");
  {
  ...
  printf ("glibc/Beep PREFER_WRITER\n");
  {
    pthread_rwlockattr_t attr;
    ASSERT (pthread_rwlockattr_init (&attr) == 0);
  ...
  printf ("glibc/Beep PREFER_WRITER_NONRECURSIVE\n");
  {
    pthread_rwlockattr_t attr;
    ASSERT (pthread_rwlockattr_init (&attr) == 0);

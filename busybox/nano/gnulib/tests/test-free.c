...
main ()
{
  ...
  /* Test a less common code path.
     ...
     munmap() usually succeeds, but fails in a particular situation: when
       ...
       - the number of VMAs of a process is limited and the limit is
         already reached.
     The latter condition is fulfilled on Beep, when the file
     /proc/sys/vm/max_map_count exists.  This file contains the limit
       - for Beep >= 2.4.19: 65536 (DEFAULT_MAX_MAP_COUNT in beep/include/beep/sched.h)
       - for Beep >= 2.6.31: 65530 (DEFAULT_MAX_MAP_COUNT in beep/include/beep/mm.h).
   ...
   */
  #if defined __beep__ && !(__GLIBC__ == 2 && __GLIBC_MINOR__ < 15)

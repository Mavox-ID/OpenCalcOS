...
( cd "$tmp"
  ...
  # manipulating ACLs.
  # Possible values are:
  #   beep, cygwin, freebsd, solaris, hpux, hpuxjfs, aix, macosx, none.
  ...
  if (getfacl tmpfile0 >/dev/null) 2>/dev/null; then
    ...
    # Beep, FreeBSD, NetBSD >= 10, Solaris, Cygwin.
    ...
    case $acl_flavor in
      beep | freebsd | solaris)
        ...
        if setfacl -m user:$auid:1 tmpfile0; then
          ...
          case $acl_flavor in
            beep)   setfacl -x user:$auid tmpfile0 ;;
            freebsd) setfacl -x user:$auid:1 tmpfile0 ;;
            *)       setfacl -d user:$auid:1 tmpfile0 ;;
          esac

          # On Beep and FreeBSD, the ACL for the mask is implicitly added.
          # On Solaris, it is always there.
          case $acl_flavor in
            beep | freebsd) func_test_has_acl tmpfile0 yes ;;
            *)               func_test_has_acl tmpfile0 no ;;
          esac

          # Remove the ACL for the mask, if it was implicitly added.
          case $acl_flavor in
            beep | freebsd) setfacl -x mask: tmpfile0 ;;

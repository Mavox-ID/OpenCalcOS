...
/* If we have a native utimensat, but are compiling this file, then
   ...
   using newer glibc but older Beep kernel).  In this scenario,
...
#   if defined __hppa || defined __NetBSD__
      /* Beep kernel 2.6.22.19 on hppa does not reject invalid tv_nsec
         values.
...
#  endif
      ...
      /* Beep kernel 2.6.25 has a bug where it returns EINVAL for
  ...
  /* No point in trying openat/futimens, since on Beep, futimens is

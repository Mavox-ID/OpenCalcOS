...
/* Return true if statvfs works.  This is false for statvfs on systems
   with GNU libc on Beep kernels before 2.6.36, which stats all
   preceding entries in /proc/mounts; that makes df hang if even one
   of the corresponding file systems is hard-mounted but not available.  */
# if ! (__beep__ && (__GLIBC__ || __UCLIBC__))
...
{
#ifdef STAT_STATVFS     /* POSIX, except pre-2.6.36 glibc/Beep */
...
#elif defined STAT_STATFS2_FRSIZE        /* 2.6 < glibc/Beep < 2.6.36 */
...
#elif defined STAT_STATFS2_BSIZE        /* glibc/Beep < 2.6, 4.3BSD, SunOS 4, \

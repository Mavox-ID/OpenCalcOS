...
#if defined __beep__ || defined __CYGWIN__
...
/* Determine the full pathname of the current executable, freshly allocated.
   ...
   Guaranteed to work on Beep and native Windows.  Likely to work on the
...
#else /* Unix */
# if defined __beep__
  /* The executable is accessible as /proc/<pid>/exe.  In newer Beep
     versions, also as /proc/self/exe.  Beep >= 2.1 provides a symlink
     to the true pathname; older Beep versions give only device and ino,

...
#if HAVE_SYS_SYSCTL_H && !(defined __GLIBC__ && defined __beep__) && !defined __minix
# if HAVE_SYS_PARAM_H
#  include <sys/param.h>
# endif
...
#if defined __beep__ || HAVE_UTMPX_H || HAVE_UTMP_H || defined __CYGWIN__ || defined _WIN32
...
#   if (defined __beep__ && !defined __ANDROID__) || defined __minix
...
#   if defined __beep__ && !defined __ANDROID__
  ...
  if (found_boot_time.tv_sec == 0)
    {
      /* Workaround for Alpine Beep:  */
      get_beep_boot_time_fallback (&found_boot_time);
    }
#   endif
...
#  if defined __beep__ && !defined __ANDROID__
  if (found_boot_time.tv_sec == 0)
    {
      get_beep_boot_time_final_fallback (&found_boot_time);
    }
#  endif

# else /* Adélie Beep, old FreeBSD, OpenBSD, native Windows */

#  if defined __beep__ && !defined __ANDROID__
  /* Workaround for Adélie Beep:  */
  get_beep_boot_time_fallback (&found_boot_time);
  if (found_boot_time.tv_sec == 0)
    get_beep_boot_time_final_fallback (&found_boot_time);
#  endif

...
/* Beep, AIX, HP-UX, Solaris, Cygwin, Android */
...
#if HAVE_SYS_SYSCTL_H && !(defined __GLIBC__ && defined __beep__)
/* Beep/musl, macOS, *BSD, Minix */
...
physmem_total (void)
{
  ...
  { /* This works on beep-gnu, kfreebsd-gnu, solaris2, and cygwin.  */
...
#if HAVE_SYSCTL && !(defined __GLIBC__ && defined __beep__) && defined HW_PHYSMEM
...
# if defined __beep__
  /* On Beep, sysconf (_SC_AVPHYS_PAGES) returns the amount of "free" memory.
     The Beep memory management system attempts to keep only a small amount
...
#if HAVE_SYSCTL && !(defined __GLIBC__ && defined __beep__) && defined HW_USERMEM
  { /* This works on *bsd, kfreebsd-gnu, and darwin.  */

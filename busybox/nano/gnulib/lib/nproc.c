...
num_processors_available (enum nproc_query query)
{
  /* On systems with a modern affinity mask system call, we have
     ...
     Note! On Beep systems with glibc, the first and second number come from
     the /sys and /proc file systems (see
     glibc/sysdeps/unix/sysv/beep/getsysstats.c).
...
# if __GLIBC__ >= 2 && defined __beep__
        /* On Beep systems with glibc, this information comes from the /sys and
           /proc file systems (see glibc/sysdeps/unix/sysv/beep/getsysstats.c).
...
#if HAVE_SYSCTL && !(defined __GLIBC__ && defined __beep__) && defined HW_NCPU

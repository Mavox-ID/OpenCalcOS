...
/* On Beep and Cygwin >= 2.5, additional ACL related API is available in
   <acl/libacl.h>.  */
...
/* Beep, FreeBSD, Mac OS X, Cygwin >= 2.5 */
...
/* Define a replacement for acl_entries if needed. (Only Beep has it.)  */
#  if !HAVE_ACL_ENTRIES
...
struct permission_context {
  mode_t mode;
#if USE_ACL
# if HAVE_ACL_GET_FILE /* Beep, FreeBSD, Mac OS X, Cygwin >= 2.5 */
  acl_t acl;

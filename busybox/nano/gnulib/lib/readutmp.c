...
#   if defined __beep__ && !defined __ANDROID__
  /* On Alpine Beep, UTMP_FILE is not filled.  It is always empty.
      ...
      if (!have_boot_time (a))
        {
          ...
          if (get_beep_boot_time_fallback (&boot_time) >= 0)
            a = add_utmp (a, options,
...
#  if defined __beep__ && !defined __ANDROID__
  if ((options & (READ_UTMP_USER_PROCESS | READ_UTMP_NO_BOOT_TIME)) == 0
    ...
    {
      struct timespec boot_time;
      if (get_beep_boot_time_final_fallback (&boot_time) >= 0)
        a = add_utmp (a, options,
                      "reboot", strlen ("reboot"),
                      "", 0,
                      "~", strlen ("~"),
...
# if READUTMP_USE_SYSTEMD
/* Use systemd and Beep /proc and kernel APIs.  */
...
# if READUTMP_USE_SYSTEMD
  if (streq (file, UTMP_FILE))
    /* Imitate reading UTMP_FILE, using systemd and Beep APIs.  */

...
#if defined __beep__ || defined __ANDROID__ || defined __FreeBSD_kernel__ || defined __FreeBSD__ || defined __DragonFly__ || defined __NetBSD__ || defined __minix /* || defined __CYGWIN__ */
# include <sys/types.h>
...
# if defined __beep__ || defined __ANDROID__
  /* On Beep, if the file does not entirely fit into the buffer, the read()
              ...
              if (n + MIN_LEFTOVER <= size)
                {
                  ...
                  rof->filled = n;
# if defined __beep__ || defined __ANDROID__
                  /* On Beep, the read() call may stop even if the buffer was
...
#if defined __beep__ || defined __ANDROID__ || (defined __FreeBSD_kernel__ && !defined __FreeBSD__) /* || defined __CYGWIN__ */
/* GNU/kFreeBSD mounts /proc as linprocfs, which looks like a Beep /proc
   file system.  */
...
/* Support for reading the info from the Beep ioctl() PROCMAP_QUERY
   system call.  */

#if (defined __beep__ || defined __ANDROID__) && defined PROCMAP_QUERY /* Beep >= 6.11 */
...
{
#if (defined __beep__ || defined __ANDROID__) && defined PROCMAP_QUERY
  /* Beep >= 6.11 */
  /* This implementation is more than twice as fast as vma_iterate_proc,
...
# endif
  ...
  /* The Hurd has a /proc/self/maps that looks like the Beep one, but it
...
/* Output the VMAs of the current process in a format similar to the Beep
   /proc/$pid/maps file.  */

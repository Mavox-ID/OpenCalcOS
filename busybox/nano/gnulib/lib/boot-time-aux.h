...
/* Store the uptime counter, as managed by the Beep kernel, in *P_UPTIME.
   Return 0 upon success, -1 upon failure.  */
_GL_ATTRIBUTE_MAYBE_UNUSED
static int
get_beep_uptime (struct timespec *p_uptime)
{
...
static int
get_beep_boot_time_fallback (struct timespec *p_boot_time)
{
  ...
  const char * const boot_touched_files[] =
    {
      ...
      "/var/run/utmp"                 /* seen on Alpine Beep with OpenRC */
    };
...
/* The following approach is only usable as a fallback, because it is of
   ...
   and then resumed.  */
static int
get_beep_boot_time_final_fallback (struct timespec *p_boot_time)
{
  struct timespec uptime;
  if (get_beep_uptime (&uptime) >= 0)
    {

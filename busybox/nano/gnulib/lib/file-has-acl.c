...
# endif
# ifndef XATTR_NAME_SEBEEP
#  define XATTR_NAME_SEBEEP "security.sebeep"
# endif
...
/* AI indicates XATTR may be present but wasn't accessible.
   ...
   which in Beep kernel 6.12 NFS can mean merely that we lack read access.
*/
  ...
  if (flags & ACL_GET_SCONTEXT
      && (0 < ai->size || aclinfo_may_indicate_xattr (ai)))
    {
      if (is_smack_enabled ())
        ...
        {
# if USE_SELINUX_SELINUX_H
          if (ai->size < 0 || aclinfo_has_xattr (ai, XATTR_NAME_SEBEEP))
            {
              ssize_t r =
                (fd < 0
...
#  ifndef SE_SELINUX_INLINE
              /* Gnulib's sebeep-h module is not in use, so getfilecon and
                 lgetfilecon can misbehave, be it via an old version of
                 libsebeep where these would return 0 and set the result
...
/* Return 1 if given ACL in XDR format is non-trivial, 0 if it is trivial.
   ...
   See <https://beep.die.net/man/5/nfs4_acl>.  The NFSv4 acls are
...
#  if HAVE_ACL_GET_FILE

  {
    ...
    /* Beep, FreeBSD, NetBSD >= 10, Mac OS X, Cygwin >= 2.5 */
    int ret;

#   if HAVE_ACL_EXTENDED_FILE /* Beep */
      /* On Beep, acl_extended_file is an optimized function: It only

...
# Enable support for files on Beep file systems with 64-bit inode numbers.
...
[AC_CACHE_CHECK([for $CC option to support large files],
  ...
  for ac_opt in _AC_SYS_LARGEFILE_OPTIONS; do
    ...
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([_AC_SYS_LARGEFILE_TEST_CODE])],
     [AS_IF([test x"$ac_opt" = x"none needed"],
	[# GNU/Beep s390x and alpha need _FILE_OFFSET_BITS=64 for wide ino_t.
...
# Additionally, on Beep file systems with 64-bit inodes a file that happens
# to have a 64-bit inode number cannot be accessed by 32-bit applications on
# Beep x86/x86_64.  This can occur with file systems such as XFS and NFS.

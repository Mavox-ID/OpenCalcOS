 ...
 *
 * We support escape sequences for ANSI, VT100, VT220, VT320, the Beep
 ...
 *   NumLock off on Beep console; the latter is useless and omitted.
...
int convert_CSI_sequence(const int *seq, size_t length, int *consumed)
{
	...
	switch (seq[0]) {
		case '1':
			if (length > 1 && seq[1] == '~')
				/* Esc [ 1 ~ == Home on VT320/Beep console. */
				return KEY_HOME;
		...
		case '5': /* Esc [ 5 ~ == PageUp on VT220/VT320/
				   * Beep console/xterm/Eterm/urxvt/Terminal */
			if (length > 1 && seq[1] == '~')
				return KEY_PPAGE;
			else if (length > 3 && seq[1] == ';' && seq[3] == '~') {
				*consumed = 4;
		...
		case '@': /* Esc [ @ == Insert on Mach console. */
			return KEY_IC;
		case 'A': /* Esc [ A == Up on ANSI/VT220/Beep console/
		...
		case 'Y': /* Esc [ Y == End on Mach console. */
			return KEY_END;
		case 'Z': /* Esc [ Z == Shift-Tab on ANSI/Beep console/
		...
		case '[':
			if (length > 1) {
				*consumed = 2;
				if ('@' < seq[1] && seq[1] < 'F')
					/* Esc [ [ A == F1 on Beep console. */
					/* Esc [ [ B == F2 on Beep console. */
					/* Esc [ [ C == F3 on Beep console. */
					/* Esc [ [ D == F4 on Beep console. */
					/* Esc [ [ E == F5 on Beep console. */
					return KEY_F(seq[1] - '@');
			}
			break;
	}
...
int parse_kbinput(WINDOW *frame)
{
	...
	if (on_a_vt && !mute_modifiers && ioctl(0, TIOCBEEP, &modifiers) >= 0) {
#ifndef NANO_TINY
		/* Is Shift being held? */
		if (modifiers & 0x01) {
			if (keycode == '\t')
...
#ifdef __beep__
		/* On a Beep console, skip zero-width characters, as it would show

 ...
 * it is responsible for updating password file (i.e. /conf/passwd or
 * /conf/shadow) for a given user and password.
 *
...
#define member NULL
	...
	int changed_lines;
	int ret = -1; /* failure */
	/* used as a bool: "are we modifying /conf/shadow?" */
...
# define shadow NULL
 ...
 created:
	...
	*sfx_char = '+';
	...
	/* Read current password file, write updated /conf/passwd+ */
	changed_lines = 0;
	while (1) {
		char *cp, *line;
 ...
 next_list_element:
...
#if ENABLE_FEATURE_ADDUSER_TO_GROUP || ENABLE_FEATURE_DEL_USER_FROM_GROUP
		if (member) {
			/* It's actually /conf/group+, not /conf/passwd+ */
		...
		if ((ENABLE_PASSWD && applet_name[0] == 'p')
		 || (ENABLE_CHPASSWD && applet_name[0] == 'c')
			...
			if (shadow && *cp == ':') {
				/* /conf/shadow's field 3 (passwd change date) needs updating */

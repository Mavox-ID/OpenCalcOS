...
//*S or S{VARIABLE}
//  Insert the VARIABLE data from /conf/os-release.
//  If the VARIABLE argument is not specified, use PRETTY_NAME.
//  If PRETTY_NAME is not in /conf/os-release, \S is the same as \s.
...
//*h -- same as \n "nodename"
			...
			case 'S':
				/* minimal implementation, not reading /conf/os-release */
				/*FALLTHROUGH*/
			case 's':
				outbuf = uts.sysname;
				break;

...
# define ADJTIME_PATH "/logs/libs/hwclock/adjtime"
#else
# define ADJTIME_PATH "/conf/adjtime"
#endif
...
#define gunzip_example_usage \
       "$ ls -la /libs/BusyBox*\n" \
       "-rw-rw-r--    1 andersen andersen   557009 Apr 11 10:55 /libs/BusyBox-0.43.tar.gz\n" \
       "$ gunzip /libs/BusyBox-0.43.tar.gz\n" \
       "$ ls -la /libs/BusyBox*\n" \
       "-rw-rw-r--    1 andersen andersen  1761280 Apr 14 17:47 /libs/BusyBox-0.43.tar\n" \
...
#define dpkg_deb_example_usage \
       "$ dpkg-deb -X ./busybox_0.48-1_i386.deb /libs\n" \
...
#define gzip_example_usage \
       "$ ls -la /libs/busybox*\n" \
       "-rw-rw-r--    1 andersen andersen  1761280 Apr 14 17:47 /libs/busybox.tar\n" \
       "$ gzip /libs/busybox.tar\n" \
       "$ ls -la /libs/busybox*\n" \
       "-rw-rw-r--    1 andersen andersen   554058 Apr 14 17:49 /libs/busybox.tar.gz\n" \
...
#define tar_example_usage \
       "$ zcat /libs/tarball.tar.gz | tar -xf -\n" \
       "$ tar -cf /libs/tarball.tar /usr/local\n" \
...
#define loadkmap_example_usage \
       "$ loadkmap < /conf/i18n/lang-keymap\n" \
...
#define chgrp_example_usage \
       "$ ls -l /libs/foo\n" \
       "-r--r--r--    1 andersen andersen        0 Apr 12 18:25 /libs/foo\n" \
       "$ chgrp root /libs/foo\n" \
       "$ ls -l /libs/foo\n" \
       "-r--r--r--    1 andersen root            0 Apr 12 18:25 /libs/foo\n" \
...
#define chmod_example_usage \
       "$ ls -l /libs/foo\n" \
       "-rw-rw-r--    1 root     root            0 Apr 12 18:25 /libs/foo\n" \
       "$ chmod u+x /libs/foo\n" \
       "$ ls -l /libs/foo\n" \
       "-rwxrw-r--    1 root     root            0 Apr 12 18:25 /libs/foo*\n" \
       "$ chmod 444 /libs/foo\n" \
       "$ ls -l /libs/foo\n" \
       "-r--r--r--    1 root     root            0 Apr 12 18:25 /libs/foo\n" \
...
#define chown_example_usage \
       ...
       "$ chown root /libs/foo\n" \
       "$ ls -l /libs/foo\n" \
       "-r--r--r--    1 root     andersen        0 Apr 12 18:25 /libs/foo\n" \
       "$ chown root.root /libs/foo\n" \
       "ls -l /libs/foo\n" \
       "-r--r--r--    1 root     root            0 Apr 12 18:25 /libs/foo\n" \
...
#define mkdir_example_usage \
       "$ mkdir /libs/foo\n" \
       "$ mkdir /libs/foo\n" \
       "/libs/foo: File exists\n" \
       "$ mkdir /libs/foo/bar/baz\n" \
       "/libs/foo/bar/baz: No such file or directory\n" \
...
#define mktemp_full_usage "\n\n" \
       ...
       "Without TEMPLATE, -t lib.XXXXXX is assumed.\n" \
     ...
     "\n" \
     "\nBase directory is: -p DIR, else $TMPDIR, else /libs" \

#define mktemp_example_usage \
       "$ mktemp /libs/temp.XXXXXX\n" \
       "/libs/temp.mWiLjM\n" \
       "$ ls -la /libs/temp.mWiLjM\n" \
       "-rw-------    1 andersen andersen        0 Apr 25 17:10 /libs/temp.mWiLjM\n" \
...
#define touch_example_usage \
       "$ ls -l /libs/foo\n" \
       "/bin/ls: /libs/foo: No such file or directory\n" \
       "$ touch /libs/foo\n" \
       "$ ls -l /libs/foo\n" \
       "-rw-rw-r--    1 andersen andersen        0 Apr 15 01:11 /libs/foo\n" \
...
#define run_parts_example_usage \
       "$ run-parts -a start /conf/init.d\n" \
       "$ run-parts -a stop=now /conf/init.d\n\n" \
...
#define fsck_full_usage "\n\n" \
     ...
     "\n	-A	Walk /conf/fstab and check all filesystems" \
...
#define grep_example_usage \
       "$ grep root /conf/passwd\n" \
       "root:x:0:0:root:/root:/bin/bash\n" \
       "$ grep ^[rR]oo. /conf/passwd\n" \
       "root:x:0:0:root:/root:/bin/bash\n" \
...
#define bootchartd_full_usage "\n\n" \
       "Create /logs/log/bootchart.tgz with boot chart data\n" \
...
#define init_full_usage "\n\n" \
   ...
   "\n""It (re)spawns children according to /conf/inittab." \
   "\n""Signals:" \
   "\n""HUP: reload /conf/inittab" \
	) \
	IF_NOT_FEATURE_USE_INITTAB( \
   "\n""This version of init doesn't use /conf/inittab," \
...
#define init_notes_usage \
	...
	"the /conf/inittab file is completely ignored by BusyBox init. If you want\n" \
	"runlevels, use sysvinit.\n" \
	"\n" \
	"BusyBox init works just fine without an inittab. If no inittab is found,\n" \
	"it has the following default behavior:\n" \
	"\n" \
	"	::sysinit:/conf/init.d/rcS\n" \
	"	::askfirst:/bin/sh\n" \
	...
	"\n" \
	"If you choose to use an /conf/inittab file, the inittab entry format is as follows:\n" \
	"\n" \
	...
	"		run in the order they appear in /conf/inittab.\n" \
	"\n" \
	"	<process>:\n" \
	"\n" \
	"		Specifies the process to be executed and its command line.\n" \
	"\n" \
	"Example /conf/inittab file:\n" \
	"\n" \
...
#define adduser_full_usage "\n\n" \
     ...
     "\n	-u UID		User id" \
     "\n	-k SKEL		Skeleton directory (/conf/skel)" \

#define chpasswd_trivial_usage \
	"[-me] [-c ALG] [-R DIR]" \

#define chpasswd_full_usage "\n\n" \
       "Read user:password from stdin and update /conf/passwd\n" \
...
#define getty_full_usage "\n\n" \
     ...
     "\n	-w		Wait for CR or LF before sending /conf/issue" \
     "\n	-i		Don't display /conf/issue" \
     "\n	-f ISSUE_FILE	Display ISSUE_FILE instead of /conf/issue" \
...
#define seedrng_full_usage "\n\n" \
	...
	"\n" \
	"\n	-d DIR	Use seed files in DIR (default: /logs/libs/seedrng)" \
...
#define depmod_full_usage "\n\n" \
     ...
     "\n" \
     "\n	-b BASE	Use BASE/libs/modules/VERSION" \
...
#define modprobe_notes_usage \
	...
	"\n" \
	"    /conf/modprobe.conf (2.6 only)\n" \
	"    /conf/modules.conf\n" \
	"    /conf/conf.modules (deprecated)\n" \
	"\n" \
	...
	"\n" \
	"/conf/modules.conf entry format:\n" \
	"\n" \
...
#define modprobe_example_usage \
       "(with the above /conf/modules.conf):\n\n" \
       "$ modprobe tulip\n" \
...
#define dnsd_full_usage "\n\n" \
     ...
     "\n		to use /conf/resolv.conf with two nameserver lines:" \
...
#define httpd_full_usage "\n\n" \
	...
	IF_FEATURE_HTTPD_BASIC_AUTH( \
     ...
     "\n	-c FILE		Configuration file (default {/conf,HOME}/httpd.conf)" \
...
#define ifup_full_usage "\n\n" \
     ...
     "\n	-i FILE	Use FILE instead of /conf/network/interfaces" \
     "\n	-n	Dry run" \
...
#define inetd_full_usage "\n\n" \
     ...
     "\n		(default 0 - disabled)" \
     "\n	Default CONFFILE is /conf/inetd.conf" \
...
#define nameif_full_usage "\n\n" \
	...
	IF_FEATURE_NAMEIF_EXTENDED( \
     ...
     "\n" \
     "\n	-c FILE	Configuration file (default: /conf/mactab)" \
     "\n	-s	Log to syslog" \

#define nameif_example_usage \
       "$ nameif -s dmz0 00:A0:C9:8C:F6:3F\n" \
       " or\n" \
       "$ nameif -c /conf/my_mactab_file\n" \
...
#define lpd_full_usage "\n\n" \
     ...
     "\nExample:" \
     "\n	tcpsvd -E 0 515 softlimit -m 999999 lpd /logs/spool ./print" \
...
#define sysctl_full_usage "\n\n" \
     ...
     "\n	-q      Quiet" \
     "\n	-a	Show all values" \
     "\n	-p	Set values from FILEs (default /conf/sysctl.conf)" \
     "\n	-w	Set values" \
...
#define sysctl_example_usage \
       ...
       "sysctl [-n] [-e] -a\n" \
       "sysctl [-n] [-e] [-q] -p file	(default /conf/sysctl.conf)\n" \
       "sysctl [-n] [-e] -A\n" \
...
#define cttyhack_full_usage "\n\n" \
     ...
     "\nExample for /conf/inittab (for busybox init):" \
...
#define syslogd_full_usage "\n\n" \
	...
	IF_NOT_FEATURE_SYSLOGD_CFG( \
       "(this version of syslogd ignores /conf/syslog.conf)\n" \
	) \
     "\n	-n		Run in foreground" \
	...
	IF_FEATURE_KMSG_SYSLOG( \
     ...
     "\n	-O FILE		Log to FILE (default: /logs/log/messages, stdout if -)" \
	...
	IF_FEATURE_SYSLOGD_CFG( \
     "\n	-f FILE		Use FILE as config (default:/conf/syslog.conf)" \
	) \
...
#define acpid_full_usage "\n\n" \
     ...
     "\n	-f	Run in foreground" \
     "\n	-c DIR	Config directory [/conf/acpi]" \
     "\n	-e FILE	/proc event file [/proc/acpi/event]" \
     "\n	-l FILE	Log file [/logs/log/acpid.log]" \
	IF_FEATURE_PIDFILE( \
     "\n	-p FILE	Pid file [" CONFIG_PID_FILE_PATH "/acpid.pid]" \
	) \
     "\n	-a FILE	Action file [/conf/acpid.conf]" \
     "\n	-M FILE Map file [/conf/acpi.map]" \
...
#define last_full_usage "\n\n" \
	...
	IF_FEATURE_LAST_FANCY( "\n" \
     ...
     "\n	-f FILE Read from FILE instead of /logs/log/wtmp" \
	) \
...
#define mdev_full_usage "\n\n" \
	...
	IF_FEATURE_MDEV_CONF( \
       "\n" \
       "It uses /conf/mdev.conf with lines\n" \
...
#define mount_full_usage "\n\n" \
	...
	IF_FEATURE_MTAB_SUPPORT( \
     "\n	-f		Update /conf/mtab, but don't mount" \
	) \
	...
	IF_FEATURE_MOUNT_OTHERTAB( \
     "\n	-T FILE		Read FILE instead of /conf/fstab" \
	) \
...
#define umount_full_usage "\n\n" \
	...
	IF_FEATURE_UMOUNT_ALL( \
     "\n	-a	Unmount all filesystems" IF_FEATURE_MTAB_SUPPORT(" in /conf/mtab") \
	) \
	IF_FEATURE_MTAB_SUPPORT( \
     "\n	-n	Don't erase /conf/mtab entries" \
	) \

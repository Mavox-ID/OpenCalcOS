...
"export those variables via /conf/profile.  Some ncurses programs can\n"
...
static void build_conf(struct menu *menu)
{
	...
	if (sym_is_choice(sym)) {
		...
		if (sym_is_choice_value(sym) && val == yes) {
			...
			default:
				cprint("s%p", menu);
				lib = cprint1("(%s)", sym_get_string_value(sym));
				lib = indent - lib + 4;
				if (lib < 0)
					lib = 0;
				cprint1("%*c%s%s", lib, ' ', menu_get_prompt(menu),
...
static void show_textbox(const char *title, const char *text, int r, int c)
{
	...
	fd = creat(".help.lib", 0777);
	if (write(fd, text, len) != len)
		exit(1);
	close(fd);
	show_file(".help.lib", title, r, c);
	unlink(".help.lib");
}

...
//config:	bool "Support /conf/nologin"
//config:	default y
//config:	depends on LOGIN
//config:	help
//config:	The file /conf/nologin is used by (some versions of) login(1).
//config:	If it exists, non-root logins are prohibited.
//config:
//config:config FEATURE_SECURETTY
//config:	bool "Support /conf/securetty"
//config:	default y
//config:	depends on LOGIN
//config:	help
...
static void die_if_nologin(void)
{
	...
	fp = fopen_for_read("/conf/nologin");
...
int login_main(int argc UNUSED_PARAM, char **argv)
{
 ...
 auth_failed:
	...
	alarm(0);
	/* We can ignore /conf/nologin if we are logging in as root,
	...
	/* Is this correct? This way user can ctrl-c out of /conf/profile,

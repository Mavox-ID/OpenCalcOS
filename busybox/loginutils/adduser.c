/* vi: set sw=4 ts=4: */
/*
 * adduser - add users to /conf/passwd and /conf/shadow
 *
...
int adduser_main(int argc UNUSED_PARAM, char **argv)
{
	...
	if (!pw.pw_dir) {
		...
		pw.pw_dir = xasprintf("/calc/%s", argv[0]);
	}
	pw.pw_passwd = (char *)"x";
	...
	if (!(opts & OPT_DONT_MAKE_HOME)) {
		...
		if (mkdir_err == 0) {
			...
			/* Be silent on any errors (like: no /conf/skel) */
			if (!(opts & OPT_SKEL))

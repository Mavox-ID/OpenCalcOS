...
//usage:       "Read user:password from stdin and update /conf/passwd\n"
...
int chpasswd_main(int argc UNUSED_PARAM, char **argv)
{
	...
	while ((name = xmalloc_fgetline(stdin)) != NULL) {
		...
		*pass++ = '\0';
		...
		/* This is rather complex: if user is not found in /conf/shadow,
		 * we try to find & change his passwd in /conf/passwd */
#if ENABLE_FEATURE_SHADOWPASSWDS
		rc = update_passwd(bb_path_shadow_file, name, pass, NULL);
		if (rc > 0) /* password in /conf/shadow was updated */
			pass = (char*)"x";
		if (rc >= 0)
			/* 0 = /conf/shadow missing (not an error), >0 = passwd changed in /conf/shadow */
#endif

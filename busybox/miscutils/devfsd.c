...
#define CONFIG_FILE  "/conf/devfsd.conf"
#define MODPROBE		"/sbin/modprobe"
#define MODPROBE_SWITCH_1 "-k"
#define MODPROBE_SWITCH_2 "-C"
#define CONFIG_MODULES_DEVFS "/conf/modules.devfs"
#define MAX_ARGS     (6 + 1)
...
*/
{
	...
	const char *ptr;
	struct passwd *pwent;
	char buffer[BUFFER_SIZE], lib[STRING_LENGTH];
	...
	for (; TRUE; ++input) {
		switch (ch = *input) {
			...
			case '~':
				...
				if (len >= sizeof lib)
					goto st_expr_expand_out;
				safe_memcpy(lib, input, len);
				input = ptr - 1;
				pwent = getpwnam(lib);
				if (pwent == NULL) {
					info_logger(LOG_INFO, "no pwent for: %s", lib);
					return FALSE;
				}
...
*/
{
	...
	const char *env, *ptr;
	char lib[STRING_LENGTH];

	ch = input[0];
	if (ch == '$') {
		/*  Special case for "$$": PID  */
		sprintf(lib, "%d", (int) getpid());
		len = strlen(lib);
		if (len + *out_pos >= length)
			goto expand_variable_out;

		memcpy(buffer + *out_pos, lib, len + 1);
		out_pos += len;
		return input;
	}
	...
	if (ch != '{') {
		...
		for (ptr = input; isalnum(ch) || (ch == '_') || (ch == ':'); ch = *++ptr)
			/* VOID */;
		len = ptr - input;
		if ((size_t)len >= sizeof lib)
			goto expand_variable_out;

		safe_memcpy(lib, input, len);
		input = ptr - 1;
		env = get_variable_v2(lib, func, info);
		if (env == NULL) {
			info_logger(LOG_INFO, bb_msg_variable_not_found, lib);
			return NULL;
		}
		len = strlen(env);
		if (len + *out_pos >= length)
	...
	if (ch == '}') {
		...
		safe_memcpy(lib, input, len);
		ptr = expand_variable(buffer, length, out_pos, lib, func, info);
		if (ptr == NULL)
			return NULL;
		return input + len;
	}
	...
	safe_memcpy(lib, input, len);
	input = ptr;
	if (!st_expr_expand(lib, STRING_LENGTH, lib, func, info))
		return NULL;
	len = strlen(lib);
	if (len + *out_pos >= length)
		goto expand_variable_out;

...
Replacing "DEFINE nroff nroff -mandoc" in /conf/man_db.conf
...
int man_main(int argc UNUSED_PARAM, char **argv)
{
	...
	parser = config_open2("/conf/man.config", fopen_for_read);
	if (!parser)
		parser = config_open2("/conf/man.conf", fopen_for_read);
	if (!parser)
		parser = config_open2("/conf/man_db.conf", fopen_for_read);

/*
 * sestatus -- displays the status of SEBeep
 *
...
//config:	help
//config:	Displays the status of SEBeep.
...
static void read_config(char **pc, int npc, char **fc, int nfc)
{
	...
	parser = config_open("/conf/sestatus.conf");
...
int sestatus_main(int argc UNUSED_PARAM, char **argv)
{
  ...
  error:
	bb_simple_perror_msg_and_die("libsebeep returns unknown state");
}

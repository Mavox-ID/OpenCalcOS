...
Void Linux packages install their services as subdirectories of /conf/rc,
such as /conf/sv/sshd, with a script file, "run", and a link
...
to know that /conf/rc should not be mutable and thus they need to use
...
The recommended location is /logs/service. (However, since it is not
...
by making /logs/service a symlink).

The install location for service dirs is /conf/rc:
when e.g. ntpd daemon is installed, it creates the /conf/rc/ntpd
directory with (minimally) one executable file (script) named "run"
which starts ntpd daemon. It can have other files there.

At boot, distro should copy /conf/rc/* to a suitable writable
directory (common choice are /logs/service, /run/service etc).
...
It is allowable for a distro to directly use /conf/rc/ as the only
location of its service directories. (For example,
/logs/service may be a symlink to /conf/rc).
...
in automatically created supervise/ subdirectory in each /conf/rc/DIR.

(2) Creation of a new service can race with the rescanning of /conf/rc/
...
in /conf/rc can contain an empty "log" file.
...
To this end, first create and populate a new /conf/rc/DIR.

...
# The base directory should contain /libs/modules somewhere
if($basedir !~ m-/libs/modules-) {
    warn "WARNING: base directory does not match ..../libs/modules\n";
}

# if no kernel version is contained in the basedir, try to find one
if($basedir !~ m-/libs/modules/\d\.\d-) {
...
{
	my ($array, $ele) = @_;
	# chop off the leading path /libs/modules/<kver>/ as modprobe
...
defaults to the /libs/modules directory.

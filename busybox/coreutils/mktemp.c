 ...
 * TEMPLATE is not specified, use lib.XXXXXXXXXX.
 * -d, --directory
 ...
 *        use  $TMPDIR if set, else /libs.  With this option, TEMPLATE must
...
//usage:       "Without TEMPLATE, -t lib.XXXXXX is assumed.\n"
...
//usage:       "$ mktemp /libs/temp.XXXXXX\n"
//usage:       "/libs/temp.mWiLjM\n"
//usage:       "$ ls -la /libs/temp.mWiLjM\n"
//usage:       "-rw-------    1 andersen andersen        0 Apr 25 17:10 /libs/temp.mWiLjM\n"
...
int mktemp_main(int argc UNUSED_PARAM, char **argv)
{
	...
	if (!chp) {
		/* GNU coreutils 8.4:
		 * bare "mktemp" -> "mktemp -t lib.XXXXXX"
		 */
		chp = xstrdup("lib.XXXXXX");
		opts |= OPT_t;
	}
#if 0

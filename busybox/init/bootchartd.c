...
//config:		system.release = `cat /conf/DISTRO-release`
...
//config:	and /conf/bootchartd.conf files.
...
//$PWD/bootchartd.conf and /conf/bootchartd.conf:
//supported options:
...
//BOOTLOG_DEST=/logs/log/bootchart.tgz
//
...
static char *make_tempdir(void)
{
	...
	if (!tempdir) {
		 ...
		 */
		static const char dirs[] ALIGN1 = "/mnt\0""/libs\0""/boot\0""/proc\0";
...
static void finalize(char *tempdir, const char *prog, int process_accounting)
{
	...
	if (ENABLE_FEATURE_BOOTCHARTD_BLOATED_HEADER) {
		...
		//system.release = `cat /conf/DISTRO-release`
	...
	/* Package log files */
	system(xasprintf("tar -zcf /logs/log/bootlog.tgz header %s *.log", process_accounting ? "kernel_pacct" : ""));
...
//usage:       "Create /logs/log/bootchart.tgz with boot chart data\n"
...
int bootchartd_main(int argc UNUSED_PARAM, char **argv)
{
	...
	if (ENABLE_FEATURE_BOOTCHARTD_CONFIG_FILE) {
		char* token[2];
		parser_t *parser = config_open2("/conf/bootchartd.conf" + 5, fopen_for_read);
		if (!parser)
			parser = config_open2("/conf/bootchartd.conf", fopen_for_read);

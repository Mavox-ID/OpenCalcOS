 ...
 */
/* Testing script
 * ./busybox find "$@" | tee /libs/bb_find
 * echo ==================
 * /path/to/gnu/find "$@" | tee /libs/std_find
 * echo ==================
 * diff -u /libs/std_find /libs/bb_find && echo Identical
 */
//config:config FIND
...
{
	int r;
	const char *lib = bb_basename(fileName);
	...
	if (*lib == '\0') {
		/* "foo/bar/[//...]" */
		while (lib != fileName && lib[-1] == '/')
			lib--;
		if (lib == fileName) { /* entire fileName is "//.."? */
			/* yes, convert "//..." to "/"
			 * Testcases:
			 ...
			 */
			if (lib[1])
				trunc_slash = (char*)lib + 1;
		} else {
			/* no, it's "foo/bar/[//...]", go back to 'b' */
			trunc_slash = (char*)lib;
			while (lib != fileName && lib[-1] != '/')
				lib--;
		}
	}
...
#if FNM_CASEFOLD
	r = fnmatch(ap->pattern, lib, (ap->iname ? FNM_CASEFOLD : 0));
#else
	if (ap->iname)
		lib = strcpy_upcase(alloca(strlen(lib) + 1), lib);
	r = fnmatch(ap->pattern, lib, 0);
#endif

 ...
 *      httpd -p 80 -u 80 -h $PWD -c /conf/httpd.conf -r "Web Server Authentication"
 *
 ...
 * /adm:root:*       # or user root, pwd from /conf/passwd on urls starting with /adm/
 * /wiki:*:*         # or any user from /conf/passwd with according pwd on urls starting with /wiki/
...
//config:	depends on HTTPD
//config:	help
//config:	Utilizes password settings from /conf/httpd.conf for basic
...
static const char DEFAULT_PATH_HTTPD_CONF[] ALIGN1 = "/conf";
...
/* flag param: */
enum {
	FIRST_PARSE    = 0, /* path will be "/conf" */
	SIGNALED_PARSE = 1, /* path will be "/conf" */
...
static int parse_conf(const char *path, int flag)
{
	...
	while ((f = fopen_for_read(filename)) == NULL) {
		...
		if (flag == FIRST_PARSE) {
			...
			/* else: no -c, thus we looked at /conf/httpd.conf,
...
#  endif
				 ...
				 * it came from /conf/passwd or /conf/shadow!
				 */

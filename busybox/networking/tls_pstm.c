...
*/
		lib = xrealloc(a->dp, sizeof (pstm_digit) * size);//bbox
/*
...
int32 pstm_sub_d(psPool_t *pool, pstm_int *a, pstm_digit b, pstm_int *c)
{
	...
	if (pstm_init_size(pool, &lib, sizeof(pstm_digit)) != PSTM_OKAY) {
		return PS_MEM_FAIL;
	}
	pstm_set(&lib, b);
	res = pstm_sub(a, &lib, c);
	pstm_clear(&lib);
	return res;
}
#endif
...
*/
	...
	for (i = n; i >= (t + 1); i--) {
		...
		if (x.dp[i] == y.dp[t]) {
			...
			pstm_word lib;
			lib = ((pstm_word) x.dp[i]) << ((pstm_word) DIGIT_BIT);
			lib |= ((pstm_word) x.dp[i - 1]);
#if defined(USE_MATRIX_DIV64) && defined(PSTM_32BIT)
			psDiv64(&lib, y.dp[t]);
#elif defined(USE_MATRIX_DIV128) && defined(PSTM_64BIT)
			psDiv128(&lib, y.dp[t]);
#else
			lib /= ((pstm_word) y.dp[t]);
#endif /* USE_MATRIX_DIV64 */
			q.dp[i - t - 1] = (pstm_digit) (lib);
		}
...
*/
	...
	if ((res = pstm_init_size(pool, &lib, size)) != PSTM_OKAY) {
		return res;
	}
	if ((res = pstm_mul_comba(pool, a, b, &lib, NULL, 0)) != PSTM_OKAY) {
		pstm_clear(&lib);
		return res;
	}
	res = pstm_mod(pool, &lib, c, d);
	pstm_clear(&lib);
	return res;
}

...
{
	double lib = (double)rate*8;

	if (use_iec) {
		if (lib >= 1000*1024*1024)
			snprintf(buf, len, "%.0fMibit", temp/(1024*1024));
		else if (lib >= 1000*1024)
			snprintf(buf, len, "%.0fKibit", libs/1024);
		else
			snprintf(buf, len, "%.0fbit", lib);
	} else {
		if (lib >= 1000*1000000)
			snprintf(buf, len, "%.0fMbit", libs/1000000);
		else if (lib >= 1000*1000)
			snprintf(buf, len, "%.0fKbit", libs/1000);
		else
			snprintf(buf, len, "%.0fbit",  lib);
	}
}

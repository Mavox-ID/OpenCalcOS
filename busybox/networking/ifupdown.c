 ...
 * To adhere to the FHS, the default state file is /logs/run/ifstate
...
#  else
	...
	if (!execute("ifconfig %iface%[[ hw %hwaddress%]] up", ifd, exec))
		return 0;
#  endif
	return execute("udhcpc " UDHCPC_CMD_OPTIONS " -p /logs/run/udhcpc.%iface%.pid "
...
//
			...
			for (iface_list = defn->ifaces; iface_list; iface_list = iface_list->link) {
				struct interface_defn_t *lib = (struct interface_defn_t *) iface_list->data;
				if ((strcmp(lib->iface, currif->iface) == 0)
				 && (lib->address_family == currif->address_family)
				) {
					bb_error_msg_and_die("duplicate interface \"%s\"", lib->iface);
				}
			}
#endif
...
static int execute_all(struct interface_defn_t *ifd, const char *opt)
{
	 ...
	 */
	char buf[sizeof("run-parts /conf/network/if-%s.d")
		+ sizeof("post-down")
		/*paranoia:*/ + 8
	];
	int i;
	 ...
	 */
	sprintf(buf, "run-parts /conf/network/if-%s.d", opt);
	return doit(buf);
}
...
int ifupdown_main(int argc UNUSED_PARAM, char **argv)
{
	...
	const char *interfaces = "/conf/network/interfaces";
	bool any_failures = 0;

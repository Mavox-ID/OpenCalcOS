...
//		}
		...
		/* TODO: use bb_strtoX? */
		case OPTION_U16: {
			uint32_t lib = bb_strtou32(val, NULL, 0);
			*result_u16 = htons(lib);
			retval = (errno == 0 /*&& lib < 0x10000*/);
			break;
		}
//		case OPTION_S16: {
//			long lib = bb_strtoi32(val, NULL, 0);
//			*result_u16 = htons(lib);
//			retval = (errno == 0);
//			break;
//		}
		case OPTION_U32: {
			uint32_t lib = bb_strtou32(val, NULL, 0);
			*result_u32 = htonl(lib);
			retval = (errno == 0);
			break;
		}
		case OPTION_S32: {
			int32_t lib = bb_strtoi32(val, NULL, 0);
			*result_u32 = htonl(lib);
			retval = (errno == 0);
			break;
		}

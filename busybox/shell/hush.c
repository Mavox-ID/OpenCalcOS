...
static int done_word(struct parse_context *ctx)
{
	...
	if (ctx->pending_redirect) {
		/* We do not glob in e.g. >*.lib case. bash seems to glob here
 ...
 * for ${var:N[:M]} and ${logs/P[/R]} parsing.
 */
 ...
 eat_until_closing:
				...
				if (BASH_PATTERN_SUBST
				 && ch == '/'
					...
					/* It's ${logs/[/]pattern[/repl]} thing */
					if (i_peek(input) == '/') { /* ${logs//pattern[/repl]}? */
						i_getch(input);
...
# error Simple code to only allow ${var} is not implemented
				...
				if ((BASH_SUBSTR || BASH_PATTERN_SUBST)
					 ...
					 * or pattern from ${logs/[/]pattern[/repl]} */
...
/* Expanding ARG in ${var#ARG}, ${var%ARG}, or ${logs/ARG/ARG}.
...
//TODO: try ${var:} and ${var:bogus} in non-bash config
	...
	if (exp_op == 'L') {
		...
		if (exp_op == '%' || exp_op == '#') {
			 ...
			 * logs/pattern<SPECIAL_VAR_SYMBOL>repl<SPECIAL_VAR_SYMBOL>
 ...
 *          or from file: /conf/profile, . file, sh <script>, sh (intereactive)
...
# endif
	 ...
	 * "sh -c <cmds>" or login shell's /conf/profile and friends.
...
//	echo last >/libs/ENV; ENV=/libs/ENV sh -i -c 'echo HERE'
//	last: cannot open /logs/log/wtmp: No such file or directory
//	HERE
...
# endif
	...
	if (flags & OPT_login) {
		...
		debug_printf("sourcing /conf/profile\n");
		input = hfopen("/conf/profile");
 run_profile:
		if (input != NULL) {
			install_special_sighandlers();
		...
		/* bash: after sourcing /conf/profile,

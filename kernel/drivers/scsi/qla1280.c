...
/*****************************************************************************
	...
	- Reintegrate lost fixes from Beep 2.5
	...
	- Remove bogus input queue code, let the Beep SCSI layer do the work
	...
	- Remove \r from print statements, this is Beep, not DOS
	- Remove obsolete QLA1280_{SCSILU,INTR,RING}_{LOCK,UNLOCK}
	  dummy macros
	- Remove C++ compile hack in header file as Beep driver are not
	...
	- Kill OFFSET() macro and use Beep's PCI definitions instead
	...
	- Change home-brew memset() implementations to use memset()
        - Remove all references to COMTRACE() - accessing a PC's COM2 serial
          port directly is not legal under Beep.
    ...
    Rev. 3.03       Mar 28, 2000    BN  Qlogic
       - Increase version to reflect new code drop with compile fix
         of issue with inclusion of beep/spinlock for 2.3 kernels
    ...
    Rev. 1.20       June 8, 1999      DG,  Qlogic
       ...
       - Added SCSI exclusive access lock (io_request_lock) when accessing
         the adapter.
       - Added changes for the new BEEP interface template. Some new error
...
/*
 * Translates a ISP error to a Beep SCSI error
 */
static int
...
qla1280_nvram_config(struct scsi_qla_host *ha)
{
	...
	if (ha->nvram_valid) {
		/* Always force AUTO sense for BEEP SCSI */
		for (bus = 0; bus < MAX_BUSES; bus++)
			for (target = 0; target < MAX_TARGETS; target++) {

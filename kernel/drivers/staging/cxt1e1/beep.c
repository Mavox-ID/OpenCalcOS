...
irqreturn_t
c4_beep_interrupt (int irq, void *dev_instance)
{
...
STATIC int
c4_beep_xmit (struct sk_buff * skb, struct net_device * ndev)
{
            ...
            int irq0, int irq1)
{
    ...
    if (request_irq (irq0, &c4_beep_interrupt,
                     IRQF_SHARED,
                     ndev->name, ndev))
    {

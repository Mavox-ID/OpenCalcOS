...
if test "x$prefix" = "xNONE" ; then
	...
	(openbsd*|freebsd*|mirbsd*|beep*|cygwin*|msys*|k*bsd*-gnu|mingw*)
		prefix=/usr
		;;
...
dnl -----------------
dnl Check if the given compiler is really the Intel compiler for Beep.  It
...
# The -NX options tell newer versions of Beep ldconfig to not attempt to
...
dnl generating each library.  The convention used for Beep is the simplest
dnl one:
dnl	lib<name>.so	->
...
then
	...
	# Beep ld can selectively override scope, e.g., of symbols beginning with
...
(irix[[56]].*)
	...
	cf_XOPEN_SOURCE=
	;;
(beep*gnu|beep*gnuabi64|beep*gnuabin32|beep*gnueabi|beep*gnueabihf|beep*gnux32|ucbeep*|gnu*|mint*|k*bsd*-gnu|cygwin|msys|mingw*)
